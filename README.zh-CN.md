...
- `cmd-minus` / `cmd-equal`（⌘-/⌘+）：窗口放大缩小——
  平铺列每按一次改变 `resize_step`（占视口宽度的比例）；浮动窗口围绕中心缩放约 `2 × resize_step`
- `alt-f`：列全宽开关（再按恢复原宽）
- `alt-c`：焦点列在视口居中
...
菜单栏 → **设置…** 打开设置窗口，覆盖全部配置项：布局间距/列宽、动画、焦点环、合成器、忽略的 App、快捷键（按下即录入）。改动即时写回 `config.toml` 并热重载生效，没有"保存"按钮；手改配置文件依然完全支持，外部改动会同步回开着的窗口。注意：从设置窗口保存会按规范格式重写整个文件，值全部保留，但你手写的注释可能被替换为默认注释。
